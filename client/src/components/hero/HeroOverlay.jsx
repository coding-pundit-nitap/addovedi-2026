...
export default function HeroOverlay() {
    ...
    // Portal entrance states
    const isEntered = useStore(state => state.isEntered);
    const portalFlash = useStore(state => state.portalFlash);
    const setIsEntered = useStore(state => state.setIsEntered);
    
    // Setters to hide original elements during transition
    ...
    const handleEnterArena = () => {
        // 1. Hide UI layers to let the cinematic warp fly-in take center stage
        setShowButton(false);
        setShowLogo(false);
        setShowNavbar(false);
        
        // 2. Trigger the camera warp speed sequence
        setIsEntered(true);
    };
                ...
                <div className="flex-1 flex flex-col items-center justify-start pt-[12vh] relative">
                    ...
                    <AnimatePresence>
                        {showButton && (
                            ...
                            >
                                <button 
                                    className="relative px-20 py-4 bg-transparent border-t-2 border-b-2 border-[#00D9FF] text-white tracking-[0.2em] font-black uppercase transition-all duration-300 hover:bg-[#00D9FF]/20 text-xl"
