/**
 * AlliancesNav.jsx — Standalone navbar for /alliances, ALLIANCES link highlighted.
 */
...
@keyframes al-scan { 0%{background-position:0 0} 100%{background-position:0 4px} }
@keyframes al-glow { 0%,100%{opacity:.75;filter:drop-shadow(0 0 6px #00D9FF)} 50%{opacity:1;filter:drop-shadow(0 0 14px #00D9FF) drop-shadow(0 0 28px #00D9FF)} }
@keyframes al-pulse { 0%,100%{box-shadow:0 0 14px rgba(0,217,255,.3),0 0 28px rgba(0,217,255,.12)} 50%{box-shadow:0 0 26px rgba(0,217,255,.6),0 0 50px rgba(0,217,255,.25)} }
.al-nav-link{position:relative;font-family:'Orbitron',monospace;font-size:.67rem;letter-spacing:.18em;text-transform:uppercase;font-weight:600;color:rgba(180,210,255,.5);padding:10px 18px;transition:color .25s;cursor:pointer;text-decoration:none}
.al-nav-link::before{content:'';position:absolute;inset:0;background:linear-gradient(135deg,rgba(0,217,255,.05),rgba(168,85,247,.05));opacity:0;transition:opacity .3s}
.al-nav-link::after{content:'';position:absolute;bottom:4px;left:50%;transform:translateX(-50%) scaleX(0);width:70%;height:2px;background:linear-gradient(90deg,#00D9FF,#A855F7);border-radius:2px;box-shadow:0 0 8px #00D9FF,0 0 16px rgba(168,85,247,.4);transition:transform .3s cubic-bezier(.34,1.56,.64,1)}
.al-nav-link:hover{color:#fff;text-shadow:0 0 10px rgba(0,217,255,.8),0 0 22px rgba(0,217,255,.3)}
.al-nav-link:hover::before{opacity:1}
.al-nav-link:hover::after{transform:translateX(-50%) scaleX(1)}
.al-nav-active{color:#00D9FF!important;text-shadow:0 0 10px #00D9FF,0 0 22px rgba(0,217,255,.6)!important}
.al-nav-active::after{transform:translateX(-50%) scaleX(1)!important;background:#00D9FF!important;box-shadow:0 0 10px #00D9FF,0 0 20px rgba(0,217,255,.6)!important}
.al-nav-active .al-c-tl,.al-nav-active .al-c-br{opacity:1!important}
.al-c-tl,.al-c-br{position:absolute;width:7px;height:7px;opacity:0;transition:opacity .3s}
.al-c-tl{top:5px;left:10px;border-top:1.5px solid #00D9FF;border-left:1.5px solid #00D9FF;box-shadow:-1px -1px 6px rgba(0,217,255,.5)}
.al-c-br{bottom:5px;right:10px;border-bottom:1.5px solid #A855F7;border-right:1.5px solid #A855F7;box-shadow:1px 1px 6px rgba(168,85,247,.5)}
.al-mob-link{position:relative;font-family:'Orbitron',monospace;font-size:.72rem;letter-spacing:.18em;text-transform:uppercase;font-weight:600;color:rgba(180,210,255,.55);padding:12px 20px;border-left:2.5px solid transparent;background:rgba(0,217,255,.02);margin-bottom:4px;transition:all .3s;cursor:pointer;text-decoration:none;display:block}
.al-mob-link:hover{color:#00D9FF;background:rgba(0,217,255,.08);border-left:2.5px solid rgba(0,217,255,.7);text-shadow:0 0 8px rgba(0,217,255,.6)}
.al-mob-active{color:#00D9FF!important;background:rgba(0,217,255,.12)!important;border-left:2.5px solid #00D9FF!important;text-shadow:0 0 10px #00D9FF!important}
`;

export default function AlliancesNav() {
    ...
    return (
        <>
            ...
            <nav style={{ position:'relative', zIndex:20, width:'100%', height:'88px', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 clamp(12px,4vw,40px)', pointerEvents:'auto' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:'12px', fontFamily:'monospace', position:'relative', zIndex:10 }}>
                    ...
                    <div style={{ display:'flex', flexDirection:'column' }}>
                        ...
                        <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                            <span style={{ width:'6px', height:'6px', borderRadius:'50%', background:'#00D9FF', boxShadow:'0 0 6px #00D9FF', animation:'al-pulse 2s ease-in-out infinite', display:'inline-block' }} />
                ...
                <div className="hidden lg:flex" style={{ alignItems:'center', position:'relative', zIndex:10 }}>
                    {NAV_LINKS.map(({ label, path }) => (
                        <a key={label} href="#" onClick={e => { e.preventDefault(); go(path); }} className={`al-nav-link${label === 'ALLIANCES' ? ' al-nav-active' : ''}`}>
                            <span className="al-c-tl" /><span className="al-c-br" />
            ...
            <AnimatePresence>
                {open && (
                    <>
                        <motion.div key="al-bd" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} onClick={() => setOpen(false)} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.75)', zIndex:40 }} className="lg:hidden" />
                        <motion.div key="al-dr" initial={{ x:'100%', opacity:0 }} animate={{ x:0, opacity:1 }} exit={{ x:'100%', opacity:0 }} transition={{ type:'spring', damping:28, stiffness:220 }} className="lg:hidden" style={{ position:'fixed', top:'96px', right:'16px', bottom:'80px', width:'290px', zIndex:50, display:'flex', flexDirection:'column', padding:'24px', gap:'24px', background:'#000', border:'1.5px solid rgba(0,217,255,.5)', backdropFilter:'blur(16px)', clipPath:'polygon(15px 0,100% 0,100% calc(100% - 15px),calc(100% - 15px) 100%,0 100%,0 15px)', boxShadow:'0 0 30px rgba(0,217,255,.25)', overflowY:'auto' }}>
                            ...
                            <div style={{ display:'flex', alignItems:'center', justifyBetween:'space-between', borderBottom:'1px solid rgba(0,240,255,.15)', paddingBottom:'16px', position:'relative', zIndex:10 }}>
                            ...
                            <div style={{ display:'flex', flexDirection:'column', position:'relative', zIndex:10 }}>
                                {NAV_LINKS.map(({ label, path }) => (
                                    <a key={label} href="#" onClick={e => { e.preventDefault(); go(path); }} className={`al-mob-link${label === 'ALLIANCES' ? ' al-mob-active' : ''}`}>
                                        {label}
                                    </a>
                                ))}
                            </div>
